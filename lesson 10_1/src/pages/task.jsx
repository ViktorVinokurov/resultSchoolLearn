...
import { deleteTaskThunk, getTaskThunk, updateTaskThunk } from '../store/tasks/api';
const Task = () => {
	const INITIOAL_DATA = {
		title: '',
		description: '',
	...
	const task = useSelector(store => store.tasksStore.task)
	...
	function loadData() {
		dispatch(getTaskThunk(params.id))
	}
	function deleteTask() {
		dispatch(deleteTaskThunk(params.id))
		navigate('/');
	}
    function finalyTask() {
		dispatch(updateTaskThunk({ ...task, finaly: true }))
		loadData()
	}
	useEffect(() => {
